...
## Bugfixes
* Wake The Dead style abilities (WTD, Army of the Dead, Necropotence, Shadow Curse) no longer create pets which can cast Fear, Gate, Shadowstep, Charm, and other problematic spell types. ( <@207640476138864641> )
...
* Spell Gem #1 is no longer disabled when using `/cast` ( <@605529525991899306> )
* An Undead Knight in the Temple of Sol Ro now appropriately responds to expected quest text. ( @MWadstein)
* Guardian of the Forest will no longer be a permanent buff duration at rank 1. ( <@145957506538012673> )
* Rev The Mysterious will now allow other Hero's to Join in on the encounter without delay. ( <@145957506538012673> )
* A goblin captain should now drop the quest item required to complete the Treant Fists quest. (<@412003659879284736>)
* Corrected various loading screen typos, and updated for newer changes. (<@412003659879284736>)
* Adjusted dialog for reaching the 7th Hammer ( <@145957506538012673> )
* Staff of the Wheel quest piece in high pass will no  longer spawn out of bounds. ( <@145957506538012673> )
* Precision timers now correctly fade buffs on their tooltip specified times. Optimized communication of buff timer remaining sent to client, so buffs timer on client matches server with precision timer. ( <@335613386949787648> )
* Fixed quest ground spawns in Kithicor Forest to no longer be out of reach of players. ( <@412003659879284736> )
...
* Mobs should now more reliably choose to path home when they lose their way.  ( <@207640476138864641> )
* Fixed incorrect damage calculation for bow damage at certain level breakpoints.
* Barter/Buyer should now correctly work across zone shards ( <@696366357247426560> )
...
* Buffs are now 'Suppressed' for 36s instead of removed when Dispelled. ( <@605529525991899306> )
* Suppressed buffs return when the suppression expires, but cannot be recast until then.
### Combat
* Heroic Strength contribution to autoattack damage now has diminishing returns over 100. ( <@207640476138864641>  )
* Heroic Dexterity now directly scales bow damage. ( <@335613386949787648> )
* Spell Effects not cast from spell gems no longer can incorrectly inherit the Heroic Intelligence or Wisdom bonus because you were casting a spell from a spell gem when the effect occurred.  ( <@207640476138864641> )
* Spell Effects from AA abilities (not procs or combat skills) now receive Heroic Intelligence or Wisdom bonuses. ( <@207640476138864641> )
* The "darkness" line of dot / snare debuffs for Shadow Knights and Necromancers will now still apply their dot if the target is immune to movement speed changes. ( <@117331286300950535> )
...
### Necromancer
*  Necromancers now have access to inherent DoT critical chance, similar to Wizard's ability to inherently crit DDs. This chance is a seperate roll for crit if the primary crit roll fails, and is not affected by any other source of crit chance. ( <@207640476138864641> )
### Wizard
* Wizard inherent critical spells now use the player's critical damage instead of a lower randomized value. They also now print a message critical informing the player their Wizard ability triggered.  ( <@207640476138864641> )
### Ranger
...
### UI
* Map Filter is no longer it's own unique window, rather integrates into the existing Map Window. ( <@543992808273608715> )
### Misc
* Jeweler Imua in the Bazaar has been practicing his enchanting and can make you an enchanted platinum bar for a modest fee. (<@117331286300950535>)
* Burrower Event Spawn timers have been reduced ( <@145957506538012673> )
* A number of NPCs in FV have had their dialogue fixed and accept multiple spell swap turn ins simultaneously ( <@218779540359479297> )
...
* Fixes to Brianna Falsrinay quest (<@412003659879284736>)
* Initiate Symbol of Brell should now be completable (<@412003659879284736>)
* Non-Bard Song spells can no longer be interrupted by Bards by spam-clicking the spell gem.
* Critical Hit messages for Melee, Skills, and Ranged attacks should now more accurately report the correct damage value (<@117331286300950535>)
...
* The Goblin King in Runnyeye can no longer double spawn. (<@412003659879284736>)
 ...
 * Lowered the time between waves for several ring events in Acrylia Caverns (<@87783602795868160>)
# Paladin
